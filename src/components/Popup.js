...
const Popup = (props) => {
let popupString = props.popup? 'h-0 transition-all ' : 'h-1/2 transition-all duration-400 ease-in-out'
...
return (
        <div className={`text-green bottom-0 fixed text-lg font-bold bg-white z-10 w-full rounded-t-3xl ${popupString}` }>
            ...
            <div className='flex flex-col items-center'>
                <button className='mt-14 absolute text-xl bg-green rounded-full text-white w-3/4 h-12' onClick={() => props.setPopup(!props.popup)}> Cancel</button>
            </div>
